...
size_t fs_read(int fd, void *buf, size_t len) {
	...
	//return -1;
	/*if(len > file_table[fd - 3].size - files[fd].offset)
	{
	...
	//Log("size = %x", size); 
	int offset = file_table[fd - 3].disk_offset + files[fd].offset;
	//Log("offset = %x", offset);
	if(files[fd].offset + len <= size - 1)
	{
		//Log("readbuf = %x, len = %x", buf, len);
	...
	{
		//Log("read");
		ide_read(buf, offset, size - 1 - files[fd].offset);
		//Log("len");
		files[fd].offset = size;
		//Log("jieshu");
		return 0;
	}
}
...
off_t fs_lseek(int fd, off_t offset, int whence) {
	...
	switch(whence)
	{
		...
		case SEEK_CUR: files[fd].offset = files[fd].offset + offset; break;
