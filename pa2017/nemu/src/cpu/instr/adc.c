...
static void instr_execute_2op() {
	...
	//else opr_dest.val = alu_adc(opr_src.val,  opr_dest.val);
	opr_dest.val = alu_adc(sign_ext(opr_src.val, opr_src.data_size),  sign_ext(opr_dest.val, opr_dest.data_size));
...
/*make_instr_func(adc_i2rm_bv){
	...
	operand_read(&i);
	operand_read(&rm);
	rm.val = alu_adc(sign_ext(i.val, data_size), sign_ext(rm.val, data_size));
	operand_write(&rm);
	return len;
}
*/
